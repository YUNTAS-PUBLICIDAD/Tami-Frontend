...
const Hero = () => {
  ...
  return (
    ...
    {/* Mobile */}
    <div className="md:hidden absolute bottom-0 left-0 w-full h-[25vh] bg-white z-10 flex flex-col justify-center items-center px-4 text-center">
      <h1
        className={`text-teal-800 text-2xl font-extrabold mb-4 transition-all duration-1000 ${
          isTextVisible
            ? "opacity-100 translate-y-0"
            : "opacity-0 translate-y-5"
        }`}
      ...
      <button
        onClick={moveToBienvenida}
        className="bg-teal-600 text-white font-bold py-2 px-6 rounded-3xl text-lg transition-all duration-300 hover:bg-teal-700"
      >
        Descubre más
      </button>
    </div>
