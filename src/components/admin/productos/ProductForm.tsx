...
const ProductForm: React.FC<ProductFormProps> = (props) => {
  ...
  const handleChange = (
    ...
    setFormData(prev => {
      ...
      if (name === "nombre") {
        nuevoEstado.nombre = value.slice(0, LENGTHS.nombre);
        if (!isLinkEdited) {
        ...
        const sanitized = value
          ...
          .toLowerCase()
          .replaceAll(" ", "-");
        nuevoEstado.link = sanitized.slice(0, LENGTHS.link);
      } else if (name === "subtitulo") {
        nuevoEstado.subtitulo = value.slice(0, LENGTHS.subtitulo);
      } else if (name === "titulo") {
        nuevoEstado.titulo = value.slice(0, LENGTHS.titulo);
      } else {
  ...
  const handleImagesTituloChange = (
    ...
    setFormData(prev => {
      ...
      nuevoArray[index] = {
        ...nuevoArray[index],
        imageTitle: e.target.value.slice(0, LENGTHS.imagenTitulo),
      };
      return { ...prev, imagenes: nuevoArray };
    });
  };
  ...
  const handleImagesTextoSEOChange = (
    ...
    setFormData(prev => {
      ...
      nuevoArray[index] = {
        ...nuevoArray[index],
        texto_alt_SEO: e.target.value.slice(0, LENGTHS.imagenAlt),
      };
      return { ...prev, imagenes: nuevoArray };
    });
  };
