...
const ChatbotWidget: React.FC = () => {
  ...
  return (
    <div className="fixed z-50 bottom-0 right-2 flex flex-col items-end">
      {/* Ventana del Chat */}
      {isOpen && (
        <div className="mb-0 w-[90vw] sm:w-[300px] h-[420px] max-h-[70vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-100 transition-all transform origin-bottom-right">

          {/* Header */}
          <div className="bg-[#00E676] px-4 py-2 text-white flex justify-between items-center shadow-md z-10">
          ...
          <div className="flex-1 p-3 overflow-y-auto bg-gray-50 flex flex-col gap-2">
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl px-3 py-1.5 shadow-sm ${msg.role === 'user'
      ...
      <button
        onClick={toggleChat}
        className={`w-16 h-16 rounded-full bg-gradient-to-tr from-[#00E676] to-[#0bc267] text-white shadow-lg flex items-center justify-center transition duration-300 ease-in-out transform hover:scale-110 ${isOpen ? 'absolute scale-90 rotate-12 opacity-0 pointer-events-none' : 'relative rotate-0 opacity-100 z-50'}`}
