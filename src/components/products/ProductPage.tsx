...
const ProductPage: React.FC<Props> = ({ producto: initialProducto }) => {
  ...
  const [productViewer, setProductViewer] = useState<string>(
    initialProducto?.imagenes?.[0]?.url_imagen ?? "/placeholder.png",
  );

  const productTitleParts = (producto?.titulo || "")
    .trim()
    .split(/\s+/)
    .filter(Boolean);
  ...
  const detailTitleColor =
    producto?.detalle_titulo_color ||
    producto?.etiqueta?.titulo_detalle_producto_color ||
    "#015f86";
  const detailTitleSize = Number(
    producto?.detalle_titulo_tamano ||
      producto?.etiqueta?.titulo_detalle_producto_size ||
      24,
  );
  const rawDetailTitleStyle =
    producto?.detalle_titulo_estilo ||
    producto?.etiqueta?.titulo_detalle_producto_style ||
    "negrita";
  ...
  const detalleProductoTituloStyle: React.CSSProperties = {
    ...
    fontWeight:
      normalizedDetailTitleStyle === "bold-italic" ||
      normalizedDetailTitleStyle === "bold"
        ? 800
        : 600,
    fontStyle:
      normalizedDetailTitleStyle === "italic" ||
      normalizedDetailTitleStyle === "bold-italic"
        ? "italic"
        : "normal",
    textDecoration:
      normalizedDetailTitleStyle === "underline" ? "underline" : "none",
    ...
    const fetchProductData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setProductViewer(
            freshProduct.imagenes?.[0]?.url_imagen ?? "/placeholder.png",
          );
      ...
      <div className="w-full min-h-screen bg-gradient-to-r from-[#041119] to-[#003d56] flex items-center justify-center text-center px-4">
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-2xl max-w-md border border-red-200">
          ...
          <p className="text-gray-600 mb-8">
            No pudimos encontrar el producto solicitado. Por favor verifica la
            URL.
          </p>
          <a
            href="/catalogo-maquinarias"
            className="bg-[#00b6ff] text-white px-8 py-3 rounded-lg font-bold shadow-lg hover:brightness-115 transition"
          >
            Volver al catálogo
          </a>
        </div>
      ...
      <div className="relative pt-32 md:pt-40  text-slate-800 overflow-hidden bg-white">
        ...
        <div className="w-full max-w-7xl mx-auto px-6 pt-8 md:px-12 z-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center relative">
          ...
          <div className="flex flex-col justify-center text-left">
            {/* Titulo y detalles*/}
            <h1
              className="text-4xl md:text-6xl font-extrabold uppercase mb-6 break-words detalle-producto-titulo-desktop"
              style={detalleProductoTituloStyle}
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8 bg-[#F0F0F0] p-5">
                <div>
                  ...
                  <p className="text-gray-500 font-medium text-sm md:text-base mb-3 italic">
                    {(producto as any).especificaciones_subtitulo ||
                      "Máximo rendimiento, estabilidad y automatización industrial"}
                  </p>
                  ...
                  <div className="flex items-center gap-8">
                    ...
                    <ul className="space-y-3">
                      {producto.dimensiones?.alto && (
                        <li>Alto - {producto.dimensiones.alto} cm</li>
                      )}
                      {producto.dimensiones?.largo && (
                        <li>Largo - {producto.dimensiones.largo} cm</li>
                      )}
                      {producto.dimensiones?.ancho && (
                        <li>Ancho - {producto.dimensiones.ancho} cm</li>
                      )}
                    </ul>
                  </div>
                </div>
              ...
              <details className="border rounded-xl bg-[#FFFFFF] shadow-sm group mt-8">
                ...
                <div className="px-6 pb-6 pt-2 max-h-[280px] overflow-y-auto pr-2">
                  <h3 className="text-gray-500 font-medium text-sm md:text-base mb-3 italic">
                    Calidad garantizada, innovación tecnológica y soporte de
                    confianza
                  </h3>
